...
| **12** | **Voice packs & render** | **Status: shipped** — see `docs/superpowers/plans/2026-07-31-voice-packs-stage12.md`. Layer 1 Phase 1: 5 voice packs (STEWARD/CINDER/PILGRIM/ASSAY/FORGE) as data files, deterministic narration renderer (no model call — see the Stage 11.5 no-cost rule), verbosity dial with a severity-3+ floor, runtime-prepended frozen phrases, interruption-budget mechanism and attention hook (both instrumented, unconsumed scaffolding). `personas.ts` is untouched — coexists, does not get folded in; see spec §5.10. Narration renders in the agent roster's DONE group, wired end-to-end through a real completion event. **Follow-up (2026-08-04), closed:** severity now reaches the roster for real — `durationBaseline.ts` tracks a per-subagent-type rolling median in-memory (session-lifetime, matching spec §8's runtime-telemetry framing) that `main.ts` snapshots before recording each completed dispatch's own duration, and `agents:narration`'s IPC payload, the reducer, and `AgentRosterCard` all carry severity through instead of a hardcoded `1` — the sev≥3 dial floor is verified reachable by test. `ROLE_MAP` in `agentVoiceRoles.ts` was also corrected against this machine's real `~/.claude/projects` transcripts: `pr-review-toolkit:code-reviewer` (44 real dispatches) had been silently falling through to FORGE, and `fork` (31 dispatches) is now mapped explicitly. **Still open at the time, closed by Stage 14:** all 4 frozen phrases were unreachable here — `formatNarration` always passed `eventKind: null`. Stage 14's `src/shared/frozenPhraseDetect.ts` adds the four predicates and `src/components/comms/narrationFeed.ts` wires them into real narration, gated on dispatch completion. Three of the four fire in production today — CINDER's `critic_tell`, ASSAY's fatal-exit `no_signal`, and STEWARD's `all_clear`. Two branches remain unreachable, named plainly rather than glossed: PILGRIM's `empty_result` and the zero-tool-calls branch of ASSAY's `no_signal` both need per-call tool-use/result data the renderer doesn't have — only tool-*call counts* reach the UI, not per-call detail — so `narrationFeed.ts` never populates those fields in production even though the predicates themselves are implemented and unit-tested. `ROLE_MAP`'s coverage was spot-verified against the highest-frequency real dispatches, not exhaustively against every possible `subagent_type`, and remains so. | 11 tasks, 2 final-review fix rounds, 2 follow-ups |
| **14** | **Comms deck** | **Status: shipped** — see `docs/superpowers/specs/2026-08-05-comms-deck-stage14-design.md`. Real transcript rendering in the Comms deck (`electron/transcriptReader.ts`, pull-based IPC — on mount, on explicit refresh, and, for a live source, re-fetched on the app's existing 900ms tick, never a `state` push), filter parsing (`transcriptFilter.ts`), and the four frozen-phrase predicates from Stage 12's open item (`frozenPhraseDetect.ts`), now narrated for real through the Stage 12 voice packs (`narrationFeed.ts`) — `interruptionBudget.ts` gets its first real consumer. `localResponder` (Stage 13.5) survives unchanged as the AETHER channel's reply path. **Privacy amendment, binding:** `docs/privacy-and-data.md` gains a render-vs-store paragraph — transcript content may be read and rendered in a mounted view but must never enter the store, persistence, `~/.aether-os/`, or the collector schema; `noPayloadInStore.test.ts` is the mechanical enforcement. This also corrects the design spec's own privacy-decision wording, which had said the IPC read happens "not on the 900ms tick" — it does, for live sources, via the existing tick; the human ruled the shipped tick-coupled behavior stands as correct and the doc's wording was amended to match. | ~6 tasks |
...
| **15** | **Cost forensics** | **Status: shipped** — see `docs/superpowers/plans/2026-08-05-cost-forensics-stage15.md`. A `Ledger` view answering "what did today cost, and which dispatch was the worst offender" from data already on this machine. **Resolves the pricing-table problem this stage was gated on:** the rates were placeholders and two of three tiers were wrong — `opus` carried the retired Opus 3 `15/75` (overstating every Opus figure the app had ever rendered by 3x) and `haiku` was `0.8/4`; operator-verified to `5/25` and `1/5` on 2026-08-07, stamped in `PRICING_VERIFIED_AT` and rendered in the view's footer so the table cannot silently age. Two further defects found while verifying and fixed in scope: cache **writes** were billed at 1.0x input rather than 1.25x, and `pricingTierForModel` fell through to `sonnet` for anything without `opus`/`haiku` in the name, billing the `fable`/`mythos` families' `10/50` at `3/15`. Also closes the Stage 12 spec's `§Gaps found in the existing codebase` item 1 — the schema-v5 dispatch columns are read into the viewer at last. **The honesty constraint is the feature:** session totals are exact and per-dispatch figures are estimates, kept as structurally distinct types (`ExactCost.usd` vs `EstimatedCost.usdApprox`) so the compiler rejects rendering one as the other; the exact-minus-estimated residual is displayed rather than normalized away; and a period with no collector data renders as an explicit gap, never `$0.00`. **Named limitation, unchanged by this stage:** spend that never passed through a transcript is invisible here — the exact shape of both incidents that motivated the work — so the view's copy scopes its claim to "observed in this machine's Claude Code transcripts" rather than implying it is the account's bill. | 5 tasks |
| **16** | **Real Projects view** | **Status: shipped** — see `docs/superpowers/plans/2026-08-07-real-projects-view-stage16.md`. Retires the last major fictional data source: `state.projects` becomes real, derived from transcript events' `cwd` values, with per-project cost and worktrees nested under their parent repo. The directory layout does not match the project roster (cwd changes mid-session); identity resolves from path strings, not filesystem probe, so deleted worktrees stay attributable in history. `unscoped` bucket expected to be large and correctly not an error state. **Out of scope:** the app-wide scope switch (making Ledger/Optimize/Analytics/Comms filter to the selected project) — intended follow-on. | 7 tasks |
| **17** | **Cross-engine Codex verification** | **Status: shipped** — see `docs/superpowers/plans/2026-08-07-codex-acp-cross-engine-verification.md` (this plan doc is also the design spec — its own §12/§15 are cited by the implementation tasks). Ships the ACP client `docs/ideas/cross-engine-verification.md` argued for (not a bespoke Codex integration): a real ACP session (`session/new`/`session/prompt`/`session/update`) against the official `codex-acp` executable, letting the operator manually ask a different model family whether a specific dispatch's claimed work is actually supported by its artifacts — a scoped, read-only file snapshot of only the touched files at the commit under test, never the live working tree. Default off; first enablement requires clicking through an explicit disclosure in the new Settings card (`CrossEngineVerificationCard.tsx`); the dispatch-level entry point is a "Verify with Codex" button in the Ledger's `DispatchCostTable`. Billing is structurally subscription-only — the child process environment strips every OpenAI billing variable, `authentication/status` is checked before every verification turn (not only at connect) and must be exactly `chat-gpt`, and there is no code path for an API key or custom gateway — enforced by `src/shared/noApiCalls.test.ts`'s cross-engine boundary suite, not just by convention. **This is the one and only outbound-network exception to the Stage 13.5 "no model call site" claim; `docs/privacy-and-data.md` §9 is amended accordingly and §1's blanket claim now names it explicitly.** **Deliberately deferred, not shipped:** automatic anomaly-triggered verification — every run in this sequence is manual, operator-initiated only; wiring verification to fire automatically off Stage 5's anomaly detectors is an explicit non-goal of this sequence (spec §15), left for a later, separately-reviewed decision. | 6 tasks |
| **18** | **Codex terminal view** | **Status: shipped** — see `docs/superpowers/plans/2026-08-09-codex-terminal-view.md`. A second, independent, real interactive `codex` CLI session (`electron/codexPtyManager.ts`, `codexPty:*` IPC, `PtyCodexTerminal`/`CodexTerminalView`) alongside the existing Claude terminal — carrying the same open-ended file/command access the Claude terminal already has, with its own independent liveness tracking (`codexTerminalAlive`) that never shares state with the Claude pty's own `PtyLifecycle` instance. Off by default (`codexTerminalCfg.enabled`), folded into the same Cross-Engine Verification settings card as Stage 17's verifier toggle rather than a new card, and gated by the same lazy, mount-triggered spawn mechanism the Claude terminal already uses — it only spawns once the operator has both enabled the toggle and navigated to the Codex sidebar view, never at app boot regardless of navigation. Shares Stage 17's `CODEX_HOME` isolation and its `OPENAI_API_KEY`/`CODEX_API_KEY` environment-stripping. **This is not a new outbound-data exception like Stage 17** — it sends no scoped snapshot to anything; it belongs in the same category `docs/privacy-and-data.md` §1 already carves out for the Claude terminal itself, documented in full at §11. **Named limitation, not glossed over:** env-stripping only closes the silently-inherited-from-shell path — it cannot stop the operator from typing an API key by hand inside the live session, the same category of limitation the Claude terminal's own key-scrubbing already has. **Carries no cost/token tracking in this version** — unlike the Ledger's Claude-side accounting, the Codex session's own token usage and cost are not observed, recorded, or surfaced anywhere in this app. | 6 tasks |
