...
| **11** | **Narration spine** | **Status: scoped, ready to implement.** Layer 1 Phase 0, mapped onto real dispatch data (Aether observes, it doesn't orchestrate) — see design: `docs/superpowers/specs/2026-07-31-narration-spine-stage11-design.md`. `task_kind`/`agent_id`/`session_id` capture, `computeSeverity()`, a fatal-via-staleness sweep for dispatches that never complete, telemetry persisted keyed by `(agent_id, task_kind)`. `retries` and `median_ms_at_eval` stay honestly unpopulated — no real signal yet. **Ships zero visible change** — no voice, no UI, nothing in the chat deck, zero `electron/`/`src/` files touched. Plan: `docs/superpowers/plans/2026-07-31-narration-spine-stage11.md`. | 7 tasks |
