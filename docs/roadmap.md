...
| **10.5** | **Spec commit & link repair** | **Status: partially done.** Task 1 (commit `AGENT_PERSONALITY_LAYER_1.md`) shipped 2026-07-30 and that link now resolves. Tasks 2-3 (commit `AETHER_MEMORY_LAYER_2.md`, track the Phase A store, repair PROGRESS.md's remaining dead link) were explicitly deferred to "a follow-up commit" that has not landed as of 2026-07-31 — the Layer 2 spec and the Phase A store (`projects/aether-layer2-phase-a/`) exist only on another machine, not in this repo or checked out anywhere on this box. Jumps the queue for the same reason Stage 0.5 did — see §3.3. Remains open; do not mark shipped until the spec file itself is committed. | ~3 tasks |
