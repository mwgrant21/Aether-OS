...
Claude U2 review carryover: include provider adapter/process-supervisor signatures and tests in this unit. Thread the private empty cwd through the PowerShell host spawn and C# CreateProcess lpCurrentDirectory, as well as thread/start; verify the real child cwd rather than only request parameters. This avoids inherited repository context but is not filesystem read confinement. Define and verify bounded retention/reaping for failed-supervision temp directories, preserving active supervision files and never treating directory removal as proof of process cleanup.
