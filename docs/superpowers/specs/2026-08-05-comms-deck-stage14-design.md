...
- A new IPC channel returns transcript messages **on request** — on view mount, on an explicit
  refresh, and, for a live source only, re-fetched on the app's existing 1s tick
...
| Push or pull? | Pull — on view mount, on an explicit refresh, and (for a live source) re-fetched on the app's existing 1s tick | A `state` push would tempt a future change into caching payload in the store. Pull keeps the payload's lifetime tied to the view's, even when the tick is what triggers the re-fetch. |
