 ...
 *
 * "Atomic" here means atomic with respect to concurrent READERS: no reader
 * ever sees a half-written file. It is NOT a durability guarantee -- neither
 * the temp file nor the directory is fsynced, so a power loss just after the
 * rename can still lose the write on some filesystems. The backup taken by
 * the caller is what bounds that, and paying two fsyncs per settings write is
